;;; ease-atoms.el --- minor mode for editing atoms input files
...
;; $Id: ease-atoms.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
(defcustom Atoms-load-hook nil
  "*Hook run when ease-atoms.el is first loaded."
  :group 'ease-programs
  :type 'hook)
	 ...
	 (if Atoms-mode
	     (progn
	       ...
	       (setq imenu-generic-expression Atoms-imenu-generic-expression
		     ...
		     ease-mouse-highlight-list Atoms-highlight-list)
		 ...
		 (condition-case ()
		   ...
		   (error nil))
		 (if (featurep 'calc)
		     (and (message "EASE: setting up calc for Atoms")
			  (require 'ease-math))))
