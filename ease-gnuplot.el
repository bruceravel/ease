;;; ease-gnuplot.el --- plotting code for EASE using gnuplot
...
;; $Id: ease-gnuplot.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
;;; Commentary:
;;  This file contains code for interfacing EASE to gnuplot.  The only
...
;;
;;  This file is used by putting a (require 'ease-gnuplot) line in any
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;; general plotting code for EASE using gnuplot

;; need to check for uwxafs binary files
...
;;;; end of general plotting code for EASE using gnuplot
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	     ...
	     ((string= arg "ksp")       ; --- chi plot ---------------------
	      (setq lines
		    (concat lines (input-gnuplot-axis-labels
				   "wavenumber" "chi(k)")
			    "set xzeroaxis\n"
			    (format "plot '%s' using ($1):($2*$1**%s)\n"
				    kspout input-k-weight))) )
...
Bound to \\[Autobk-plot-all-chi]"
  ...
  (save-excursion
	...
	(if (input-empty-stanza-p)
	    (input-previous-stanza 1)
	  (Autobk-parse-stanza)
	  (add-to-list 'ksplist (cdr (assoc "kspout" Autobk-features-alist)))
	  (input-previous-stanza 1)))
      (Autobk-parse-stanza)
      (setq ksplist (cons (cdr (assoc "kspout" Autobk-features-alist)) ksplist))
      (setq lines                       ; write script
	    (concat (input-gnuplot-cd-and-title
		    ...
		    "set xzeroaxis\n"
		    (format "plot '%s'  using ($1):($2*$1**%s)"
			    (nth 0 ksplist) input-k-weight)))
      (setq input-current-terminal input-gnuplot-terminal)
      (while (< counter (length ksplist)) ; write each element of list
	(cond ((not (nth counter ksplist))
	       (setq counter (1+ counter))) ; weed out nil values
	      ((string-match "[ \t]+"(nth counter ksplist))
	       (setq counter (1+ counter))) ; weed out blank values
	      (t
	       (setq lines
		     (concat lines
			     (format ",\\\n     '%s'  using ($1):($2*$1**%s)"
				     (nth counter ksplist) input-k-weight)))))
...
All marked paths will also be plotted.
Bound to \\[Feffit-plot-k]."
  (interactive)
  (let (logical outbase)
    ...
    (cond (outbase
	   ...
	   (if (file-exists-p (concat outbase "k.dat"))
	       (Feffit-plot "k")
	     (message "Cannot plot k-space fit.  %sk.dat does not exist." outbase)))
	  (t
...
Bound to \\[Feffit-plot-r]."
  ...
  (let (logical outbase)
    ...
    (setq logical (cdr (assoc "rspout" Feffit-features-alist))
	  outbase (cdr (assoc "outbase" Feffit-features-alist)))
    (cond (outbase
	   (if (file-exists-p (concat outbase "r.dat"))
	       (Feffit-plot "r")
	     (message "Cannot plot R-space fit.  %sr.dat does not exist." outbase)))
	  (t
	   (message "No output filename is provided in this input file.")) ) ))
...
Bound to \\[Feffit-plot-q]"
  ...
  (let (logical outbase)
    ...
    (setq logical (cdr (assoc "qspout" Feffit-features-alist))
	  outbase (cdr (assoc "outbase" Feffit-features-alist)))
    (cond (outbase
	   (if (file-exists-p (concat outbase "q.dat"))
	       (Feffit-plot "q")
	     (message "Cannot plot q-space fit.  %sq.dat does not exist." outbase)))
	  (t
	   (message "No output filename is provided in this input file.")) ) ))

(defconst ease-plot-space-stuff
...
and optionally write out the background and all of the individual paths."
  ...
  (let (lines indeces index (offset 2) (input-buffer (current-buffer))
	...
	(kw      (elt (cdr (assoc space ease-plot-space-stuff)) 0))
	(ext     nil)
	(mr      nil)
	(using   (elt (cdr (assoc space ease-plot-space-stuff)) 4))
	(column  (eval (elt (cdr (assoc space ease-plot-space-stuff)) 5)))
	(complex (elt (cdr (assoc space ease-plot-space-stuff)) 6))
	(axes    (elt (cdr (assoc space ease-plot-space-stuff)) 7)) )
    (when (string= formout "uwxafs")	; handle uwxafs, kweight, allout
      (setq kw  (elt (cdr (assoc space ease-plot-space-stuff)) 3)
	    ext (elt (cdr (assoc space ease-plot-space-stuff)) 1)
	    mr  (elt (cdr (assoc space ease-plot-space-stuff)) 2) ))
    (if kw (setq kw (format kw input-k-weight)))
    (if (equal space "k") (setq using (format using input-k-weight)))
    ;;(if allout
    ...
    (setq lines				; beginning of gnuplot script
	  ...
	  input-current-terminal input-gnuplot-terminal)
    (if complex
	(setq lines (concat lines "n=" (number-to-string column)
    ...
    (cond ((string= formout "ascii")	; lines for data, fit, bkg, paths
	   ...
	   (if (and bkgout (file-exists-p (concat outbase space ".bkg")))
	       (setq lines (concat lines
...
;;; additions to gnuplot mode for ease.  the script history gets
;;; placed onto the `gnuplot-after-run-hook'

(defcustom ease-gnuplot-beep-flag input-beep-flag
  "*Non-nil means to beep when running into the ends of the history."
  :group 'ease-features
  :group 'gnuplot
  :type 'boolean)
(defcustom ease-gnuplot-history-length 10
...
as a function in `gnuplot-after-plot-buffer-hook',"
  ...
  :group 'gnuplot
  :type 'integer)
(defcustom ease-gnuplot-top-bottom-separator "^#-#-#-"
...
(defvar ease-gnuplot-used nil)
(defvar ease-gnuplot-history () "Script history list for gnuplot.")
(defvar ease-gnuplot-history-index -1 "Current location in gnuplot history list.")

(define-key gnuplot-mode-map [(control c) down] 'ease-gnuplot-next-script)
(define-key gnuplot-mode-map [(control c) up]   'ease-gnuplot-previous-script)

(defun ease-gnuplot-menubar-xemacs ()
  (add-menu-button '("Gnuplot") "---")
  (add-menu-button '("Gnuplot") "Gnuplot functions for EASE")
  (add-menu-button '("Gnuplot")
		   ["previous script in history" ease-gnuplot-previous-script t])
  (add-menu-button '("Gnuplot")
		   ["next script in history" ease-gnuplot-next-script t]))

(defun ease-gnuplot-menubar-emacs ()
  ...
  (define-key-after (lookup-key gnuplot-mode-map [menu-bar Gnuplot])
    [easelabel]
    '("Gnuplot functions for EASE" . nil)
    [justaline])
  (define-key-after (lookup-key gnuplot-mode-map [menu-bar Gnuplot])
    [previous]
    '("previous script in history" . ease-gnuplot-previous-script)
    [easelabel])
  (define-key-after (lookup-key gnuplot-mode-map [menu-bar Gnuplot])
    [next]
    '("next script in history" . ease-gnuplot-next-script)
    [previous]))

(cond ((string-match "XEmacs" emacs-version)
       (add-hook 'gnuplot-mode-hook 'ease-gnuplot-menubar-xemacs))
      (t
       (add-hook 'gnuplot-mode-hook 'ease-gnuplot-menubar-emacs)))


;;; script history utilities

(defun ease-gnuplot-push-history (string)
...
character string."
  (interactive)
  (let ((l (length ease-gnuplot-history)))
    (if ease-gnuplot-history
	(setq ease-gnuplot-history
	      (append (list string) ease-gnuplot-history))
      (setq ease-gnuplot-history (list string)))
    (and (> l ease-gnuplot-history-length)
	 (setq ease-gnuplot-history (butlast ease-gnuplot-history 1))) )
  t)

(defun ease-gnuplot-previous-script ()
  "Step back in gnuplot history.
Bound to \\[ease-gnuplot-previous] in the gnuplot buffer."
  (interactive) (ease-gnuplot-navigate-history 1))

(defun ease-gnuplot-next-script ()
  "Step forward in gnuplot history.
Bound to \\[ease-gnuplot-next] in the gnuplot buffer."
  (interactive) (ease-gnuplot-navigate-history -1))

(defun ease-gnuplot-navigate-history (place)
...
forward (element 0 in the list is the most recent)."
  (let ((buffer (current-buffer)))
    (cond ((not (or (equal place -1) (equal place 1)))
	   nil)
	  ((equal 1 (length ease-gnuplot-history))
	   (if ease-gnuplot-beep-flag (ding))
	   (message "This is the only entry in the gnuplot history."))
	  ((and (equal place -1) (<= ease-gnuplot-history-index 0))
	   (if ease-gnuplot-beep-flag (ding))
	   (message "This is the most current entry in the gnuplot history."))
	  ((and (equal place 1) (equal (+ 1 ease-gnuplot-history-index)
				       (length ease-gnuplot-history)))
	   (if ease-gnuplot-beep-flag (ding))
	   (message "This is the oldest entry in the gnuplot history."))
	  (t
	   ;;(switch-to-buffer gnuplot-script-buffer-name)
	   (goto-char (point-max))
	   (search-backward (regexp-quote ease-gnuplot-top-bottom-separator)
			    (point-min) "to_limit")
	   (and (looking-at (regexp-quote ease-gnuplot-top-bottom-separator))
		(forward-line 1))
	   (and (equal place 1)
		(equal ease-gnuplot-history-index -1)
		(setq ease-gnuplot-history-index 0))
	   (delete-region (point) (point-max))
	   (setq ease-gnuplot-history-index (+ ease-gnuplot-history-index place))
	   (insert (elt ease-gnuplot-history ease-gnuplot-history-index))
	   (switch-to-buffer buffer)) )))


(defun ease-gnuplot-after-plot-buffer ()
  "Update history list after plotting the buffer."
  (save-excursion
    (when (equal 'buffer gnuplot-recently-sent)
	  (if (search-backward (regexp-quote ease-gnuplot-top-bottom-separator)
			       (point-min) "to_limit")
	      (forward-line 1))
	  (ease-gnuplot-push-history
	   (buffer-substring-no-properties (point) (point-max)))
	  (setq ease-gnuplot-history-index -1))))
...
(defun ease-amend-gnuplot-toolbar ()
  (fset 'gnuplot-up-fn   'ease-gnuplot-previous-script)
  (fset 'gnuplot-down-fn 'ease-gnuplot-next-script)
...
(add-hook 'gnuplot-after-plot-hook 'ease-gnuplot-after-plot-buffer)
;; need to tidy up a few things so gnuplot mode plays well with ease
(add-hook 'gnuplot-mode-hook
	  '(lambda ()
	     ...
	     (fset 'gnuplot-kill-gnuplot 'ease-kill-gnuplot)
	     (and (featurep 'toolbar)
		  (ease-amend-gnuplot-toolbar))))


;;; That's it! ----------------------------------------------------------------

; any final chores before leaving
(provide 'ease-gnuplot)

;;;============================================================================
;;;
;;; ease-gnuplot.el ends here
