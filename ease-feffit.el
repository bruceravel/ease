;;; ease-feffit.el --- minor mode for editing feffit input files
...
;; $Id: ease-feffit.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
(defcustom Feffit-load-hook nil
  "*Hook run when ease-feffit.el is first loaded."
  :group 'ease-programs
  :type 'hook)
...
(defun ease-Feffit-choose-template ()
  (interactive)
...
Bound to \\[Feffit-renumber-paragraph]"
  ...
  (save-excursion
	...
	(if (input-comment-p)
	  ...
	  (insert (concat space (number-to-string index) space "."))
...
;; Something like this would be very useful in .ease:
;;   (define-key input-mode-map [(f11)] 'Feffit-quick-plot-path)
;; This could be in ease-gnuplot, but should another plotting interface
...
See `Feffit-mark-path-for-plot'.  Bound to \\[Feffit-clear-all-marked-paths]"
  (interactive)
  (ease-clear-overlays 'face 'Feffit-path-face)
  (setq Feffit-marked-paths ()))
...
(defun Feffit-insert-this-best-fit (&optional dont-swap)
  "Find and replace the guess value on the current line with its best fit.
Also include uncertainty as an end of line comment.  Unless DONT-SWAP is
non-nil, change the \"guess\" keyword to \"set\".
Bound to \\[Feffit-insert-best-fit]"
  (interactive "*P")
  (save-excursion
    (let (word value error expr space pnt mark-1 basename logname
	       (case-fold-search t)
	       (buff (current-buffer)))
      (if input-master
	  (setq basename input-master)
	...
	(setq expr (concat "^[ \t]+" (regexp-quote word) "[ \t]+="))
	(setq pnt  (point-marker))
	;;(setq expr (concat "^[ \t]*\|?[ \t]*\\<" (regexp-quote word) "\\>"))
	(message "%s" buff)
	(cond ((file-exists-p logname)
	       ...
	       (cond ((re-search-forward expr (point-max) t)
		      ...
		      (insert (format "%s%s  \t!  +/- %s    -- best fit value"
				      space value error))
		      (or dont-swap (input-guess-to-set)))
		     (t
	 ...
	 (if Feffit-mode
	     (progn
	       ...
	       (setq imenu-generic-expression Feffit-imenu-generic-expression
		     ...
		     ease-mouse-highlight-list Feffit-highlight-list)
