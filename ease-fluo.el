;;; ease-fluo.el --- minor mode for editing fluo input files
...
;; $Id: ease-fluo.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
(defcustom Fluo-load-hook nil
  "*Hook run when ease-fluo.el is first loaded."
  :group 'ease-programs
  :type 'hook)
...
(when (and ease-xemacs-p (featurep 'toolbar))
  ...
  (defvar Fluo-toolbar
    '([ease-toolbar-template-xpm   Fluo-toolbar-template
				   t "Write an Fluo template"]
      [ease-toolbar-run-xpm        Fluo-toolbar-run
				   t "Run Fluo"]
      [:style 3d :size 8]
      ...
      [:style 3d :size 8]
      [ease-toolbar-helper-xpm     Fluo-toolbar-helper
				   t "Display Fluo keywords"]
      [ease-toolbar-quit-xpm       Fluo-toolbar-quit
  ...
  (add-hook 'Fluo-mode-hook
	    '(lambda ()
	       (and (featurep 'toolbar)
		    (ease-make-toolbar-function Fluo-toolbar))))
  ...
  (fset 'Fluo-toolbar-quit     'ease-quit)  )
;;----------------------------------------------------------------------
	 ...
	 (if Fluo-mode
	     (progn
	       ...
	       (setq imenu-generic-expression Fluo-imenu-generic-expression
		     ...
		     ease-mouse-highlight-list Fluo-highlight-list)
