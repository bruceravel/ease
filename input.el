;;; input.el --- major mode for editing FEFF, FEFFIT, and other input files
...
;; $Id: head.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
;; or
;;   ftp://atropos.nist.gov/pub/bruce/ease/ease-#.#.#.tar.gz
...
;; the features of EASE (running programs, plotting) will never work under
;; DOS and may fail under Win95.

;;                      INSTALLATION INSTRUCTIONS
;;
;; Most of these installation instructions are handled by a
;; configuration script and by the makefile distributed with EASE.
...
;; enable EASE when editing input files with emacs.  Those lines will
...
;;
;; If you add these lines by hand be sure to replace "~/lisp/ease" by
;; the location of your EASE installation.
;;
;; See the document for installing EASE by hand.
;;

;;                        DESCRIPTION OF PROGRAM
;;
;; EASE is an emacs package for editing the input files to FEFF,
;; FEFFIT, and other programs.  It contains functions for running the
;; codes from within emacs, plotting the output, generating input
...
;;
;; This is not a good place to describe all of the features of EASE.
;; For that, I refer you to the documentation, which is available in a
;; variety of forms, including postscript, info, and html.  In EASE,
...
(defvar ease-xemacs-p (string-match "XEmacs" emacs-version)
  "Non-nil if this is XEmacs.")
(defvar ease-gtk-xemacs-p (string-match "GTK" emacs-version)
  "Non-nil if this is GTK-XEmacs.")
...
(autoload 'ease-mode     "ease"     "ease-mode"     t)
(autoload 'ease-quit     "ease"     "quit editing an input file" t)
(autoload 'ease-doc-mode "ease-doc" "ease-doc-mode" t)

(eval-when-compile (defvar Feff-comment-alist ()))
;; $Id: user-custom.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
(defgroup ease-appearance nil
  "Variables controlling the appearance of EASE on the screen."
  :prefix "ease-"
  :prefix "input-"
  :group 'ease)
(defgroup ease-indentation nil
  "Indentation and separation parameters."
  :prefix "ease-"
  :prefix "input-"
  :group 'ease)
(defgroup ease-features nil
  "Variables for EASE's interfaces to other Emacs packages and to
external processes."
  :prefix "ease-"
  :prefix "input-"
  :group 'ease)
(defgroup ease-programs nil
  "Program specific variables in EASE."
  :prefix "ease-"
  :prefix "input-"
  :group 'ease)
...
This is run very early on in EASE's execution wrapper.  It comes
...
particular programs should contain conditionals."
  ...
  :type 'hook)
					...
					; directory paths
(defcustom ease-base-directory  (expand-file-name "/home/bruce/source/ease/")
  "*Installation directory for EASE.
If this is set incorrectly, emacs will not be able to find documentation,
scripts needed for plotting, or toolbar icons."
  :group 'ease
  :type 'file)

(defcustom input-bin-location             (concat ease-base-directory "scripts/")
  "*Installation location of the scripts and programs distributed with EASE.
This must be set correctly to use things like *kw*, *mr*, and *intrp*.  By
default this is under `ease-base-directory'."
  :group 'ease
  :type 'file)
(defcustom input-document-location        (concat ease-base-directory "docs/")
  "*Installation location of the EASE document.
This must be set correctly to use the on-line EASE document with
\\[input-document].  The forms are info, html, and plain text.  By
default this is under `ease-base-directory'."
  :group 'ease
  :type 'file)
(defcustom input-program-document-location (concat ease-base-directory "docs/")
...
default this is under `ease-base-directory'."
  :group 'ease
  :type 'file)
;; (defcustom ease-glyph-directory           (concat ease-base-directory "pixmaps/")
...
(defcustom ease-shell "bash"
  "Shell to use to launch asynchronous subprocesses.  Bash is a good choice."
  :group 'ease
  :type 'string)
(defcustom ease-shell-switches "-c"
  "Shell command line switches used to lauch asynchronous subprocesses.
These MUST be appropriate to the shell specified by `ease-shell'."
  :group 'ease
  :type 'string)

					; appearance customization
(defcustom ease-inhibit-startup-message nil
  "*Non-nil means to not display startup messages."
  :type 'boolean
  :group 'ease-appearance)

(defcustom ease-use-toolbar (if (featurep 'toolbar)
				'left-toolbar
			      nil)
...
or `top-toolbar' may be a bad idea since either will make the EASE toolbar
...
(defvar input-stanza-delimiter
  "--------------------------------------------------------"
  "*Stanza delimiter in several programs.")
;;  :group 'ease-appearance
;;  :type 'string)
...
(defcustom input-init-file                  "~/.ease"
  "*Default name of the EASE initialization file."
  :group 'ease-appearance
  :type 'file)
					; run-time logfile customization
(defcustom input-run-log-interactive        "ease-run.log"
  "*Default filename for an interactively saved run-time log.
By default this goes into the current directory."
  :group 'ease-appearance
  :type 'file)
(defcustom input-run-log                    "~/.ease-run.log"
  "*Default filename for the run-time log saved when Emacs is exited.
This file serves as a record of your previous session with EASE."
  :group 'ease-appearance
  :type 'file)
...
(defcustom input-stanza-name                "ease-stanza.inp"
...
These situations are the end of running the program and other long actions."
  :group 'ease-appearance
  :type 'boolean)
(defcustom ease-mouse-overlays-default-flag t
  "Initial setting for mouse overlays in EASE."
  :group 'ease-appearance
  :type 'boolean)
...
If nil, then EASE will leave a selected frame in its current state."
  :group 'ease-appearance
  :type 'boolean)

(defcustom input-doc-frame-plist
    (append (list 'width 80) (list  'height 30))
...
(defcustom input-run-frame-plist
  '(minibuffer t height 17 width 69 border-width 0
  ...
  :type '(repeat (group :inline t
			...
			(sexp :tag "Value")))
  :group 'ease-appearance)
(defcustom input-run-frame-parameters '((minibuffer . t)
					(height . 17)
					(width . 69)
					(border-width . 0)
					(menu-bar-lines . 0)
...
(defcustom input-gnuplot-frame-plist
  '(minibuffer t height 21 width 66 border-width 0
	       internal-border-width 0 unsplittable t
	       default-toolbar-visible-p nil has-modeline-p nil
	       menubar-visible-p t)
...
;;   "*Editor emulation mode for use with EASE.
;; This variable is used to assure that an emulation mode is entered when
;; EASE is started.  The values are \"vi\", \"crisp\", and \"edt\"."
;;   :group 'ease-features
...
(defcustom gnuplot-script-buffer-name       "ease.gp"
...
(defcustom input-gnuplot-default-ps-file    "ease.ps"
...
(defvar input-document-type "info")
...
sequence of characters.  EASE ends the time stamp string with a `>'"
  :group 'ease-features
  :type 'string)
...
(defcustom input-time-stamp-format "%:y/%02m/%02d %02H:%02M:%02S %u"
  "*Format used for time stamp.
See document for `time-stamp-format'."
  :group 'ease-features
  :type 'string)


(defcustom input-use-imenu                  "Imenu"
  "*If non-nil this is the Imenu name in the menubar.
This is ignored for XEmacs."
  :group 'ease-features
  :type 'string)
...
(cond ((and (featurep 'custom) (fboundp 'custom-declare-variable))
       (defface ease-mouse-face '((((class color))
				   (:background "orange"))
				  (t
				   (:underline t)))
	 "Mouse face used when mouse over a readable filename."
	 :group 'ease-appearance)
       (defface ease-hotspot-face '((((class color))
				   (:background "salmon"))
				  (t
				   (:reverse t)))
	 ...
	 :group 'ease-appearance))
      (t
       (copy-face 'highlight 'ease-mouse-face)
       (set-face-background  'ease-mouse-face   "orange")
       (copy-face 'modeline  'ease-hotspot-face)
       (set-face-background  'ease-hotspot-face "salmon")
...
(defun input-customize ()
  "Customize EASE using the Custom package."
  (interactive)
  (if (featurep 'custom)
      (customize-group 'ease)
...
;; $Id: variables.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
be one entry for each minor mode in *EASE*.  The car is the program
...
;;   "List of editor emulators known to EASE available in Emacs.")

;;;; end of variables to be set at installation time
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

;;;; constants
(defconst input-mode-version "0.6.5")
...
;;(defconst input-edges "\\<k\\|l[123]\\|[mno][1-5]|[no][67]|p[1-3]\\>")
...
(defvar input-current-keywords-alist ()
  "This alist is set to the keyword alist for the current
...
(defvar ease-mouse-highlight-list '()
...
(defvar ease-mouse-overlays-flag nil)
(setq ease-mouse-overlays-flag ease-mouse-overlays-default-flag)
...
(defvar input-batch-files ()
  "List of marked files for a EASE batch job.
Don't change this by hand!")
...
(defvar ease-running-this-file nil)

(defvar ease-startup-displayed nil
  "Non-nil means that the startup messages have already been displayed.")
(defvar ease-startup-message-lines
    '("EASE: EXAFS Analysis System for Emacs"
      "Please use \\[input-submit-feedback] to report bugs."
      "Use \\[input-ease-document] to read the EASE document."
      "Use \\[input-display-keywords] to see descriptions of program keywords."
      "EASE comes with absolutely no warranty."
      "Feel free to distribute copies of EASE."
      "Thanks for using EASE.  Have fun!"))
;;       "You may give out copies of EASE.  Type \\[vm-show-copying-restrictions] to see the conditions"
...
;; $Id: bindings.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
(if input-mode-map
  ...
  (define-key input-mode-map "\C-c\C-tc" 'ease-clear-hotspot-overlays)
  ...
  (define-key input-mode-map "\C-c\C-dz"  'input-insert-z-number)
  ...
  (define-key input-mode-map "\C-c\C-fk"  'ease-kill-gnuplot)
  ...
  (define-key input-mode-map "\C-c\C-be"  'input-ease-document)
  (define-key input-mode-map "\C-c\C-bu"  'input-ease-tutorial)
  (define-key input-mode-map "\C-c\C-bk"  'input-display-keywords)
  (define-key input-mode-map "\C-c\C-bl"  'input-save-run-log)
  (define-key input-mode-map "\C-c\C-bm"  'ease-display-startup-message)
  ...
  (define-key input-mode-map "\C-c\C-bt"  'ease-doc-mode)
...
(easy-menu-define
 ...
 '("Input"
   [" >> EASE <<"                          ease-mode t]
   ;;:active (featurep 'ease)]
   ("Toggled features"
    ["Ease-Doc mode"                       ease-doc-mode
     :style toggle :selected (and (featurep 'ease-doc) ease-doc-mode)]
    ["Mouse highlighting in this file"     ease-toggle-mouse-overlays
     :style toggle :selected ease-mouse-overlays-flag]
    ["Hide autoconfig variables"           ease-toggle-hide-autoconfig
     :style toggle :selected ease-hide-autoconfig-flag])
   ("Editing Shortcuts"
    ...
    ["Insert a Z number at point"          input-insert-z-number t]
    )
   ...
   ("Keyword operations"
    ...
    ["Previous template hotspot"           tempo-backward-mark
     :active input-used-tempo-flag]
    ["Clear hotspot-markings"              ease-clear-hotspot-overlays
     :active input-used-tempo-flag])
   ...
   ("Gnuplot"
    ...
    ["Kill gnuplot buffer and process"     ease-kill-gnuplot
     :active (and gnuplot-buffer (bufferp (get-buffer gnuplot-buffer)))] )

   ("Miscellaneous"
    ["Customize EASE"                      input-customize
    ...
    ["Refresh mouse overlays"              ease-set-all-mouse-overlays t]
    ...
    ["Submit EASE bug report"              input-submit-feedback t]
    ["Submit program bug report"           input-submit-program-bug-report t]
    ["Display startup message sequence"    ease-display-startup-message t]
    ["Show EASE version number"            input-show-version t])

   ;;["Documentation"                        input-document t]
   ("Documentation"
    ["EASE Document"                       input-ease-document t]
    ["EASE Tutorial"                       input-ease-tutorial t])
...
;; $Id: keywords.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
defined program.  If found, write a message to the echo area about
what kind of argument it takes.  Bound to
\\[input-input-complete-keyword]"
  (interactive)
...
;; $Id: templates.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
Uses completion.  Bound to \\[input-insert-filename]"
  (interactive)
  (insert (file-relative-name (read-file-name "Filename > " "")) ))

(defun input-insert-z-number (&optional atom)
...
;; $Id: swap-indent.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $

;; functions defined in this section
;;    input-guess-to-set ()
;;    input-true-false ()
;;    input-swap-comment (&optional arg)
...
;; functions defined in this section
;;    input-indent (&optional location)
...
;; $Id: run.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
This function appears in each of the minor mode pull-down menus in EASE.
...
`input-before-run-hook' is run before begining program execution.  For
a batch job, it is run before each execution.
...
to a buffer in another frame."
  (let ((program (elt list 0)) (file (elt list 1)) (version (elt list 2))
    ...
    (cond ((string= (process-status input-process) "run")
	   (message
  "You already have a process running.  EASE only allows one at a time."))
	  ;; be really pedantic...
	  ...
	  (t
	   ...
	   (when (and input-use-frames (not ease-gtk-xemacs-p))
		 (raise-frame (selected-frame)))
	   (if input-batch-flag (save-buffer)
	     (save-some-buffers))
	   (setq input-temp-string (make-temp-name "EASE")
		 input-process-directory
		 (file-name-directory (expand-file-name file))
		 input-basename
		 (file-name-sans-extension (file-name-nondirectory file))
		 ease-running-this-file t)
	   ...
	   (setq input-program-running      program
		 ...
		 input-original-frame       (selected-frame))
	   ...
	   (if input-use-frames         ; display run in another frame?
	       (and (select-frame frame) (switch-to-buffer buffer))
...
PROCESS and EVENT are normal sentinel arguments."
  (let (filename outfile character ;; (buffer (current-buffer)))
	...
	(cond ((string= "." (substring outfile 0 1))
	       (setq filename (concat input-process-directory
				      input-program-running outfile))
		 ...
		 (rename-file filename
	      ...
	      (t
	       (setq filename (concat input-process-directory outfile))
	       (if (file-exists-p filename)
		   (rename-file filename
				(concat input-process-directory
					input-basename "-" outfile)
...
;; 			       input-program-running)))
    ...
    (setq ease-running-this-file nil)
    ...
    (cond (input-batch-flag
	   ...
	   (cond ((= (length input-batch-files) input-batch-index)
 		  (setq input-batch-index  0
			input-batch-files ()
			input-batch-flag nil
			input-started-by-batch-flag nil)
    ...
    (setq input-temp-string          nil
	  input-process-directory    nil
	  input-basename             nil
	  ease-running-this-file     nil
	  input-program-running      nil
	  input-file-buffer          nil
	  input-current-output-files nil
	  input-original-frame       nil) ))
...
(defun input-kill-program ()
  "Kill the currently running EASE process.
...
processes.  Bound to \\[input-kill-program]"
  ...
  (let ((frame (selected-frame)))
	...
	(error "There is not an EASE process currently running.")
...
;; $Id: movement.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
(defconst input-stanza-separate "^[!%#* \t]*------+"
  "Job delimiter in some programs.
...
(defun input-empty-stanza-p ()
  "Returns t if point is currently in a space that is not really a stanza."
  (let ((beg (save-excursion (input-beginning-of-stanza) (point)))
        (end (save-excursion (input-end-of-stanza)       (point))))
    (< end beg)))
...
(defun input-end-of-paragraph ()
  "Move to the first blank line after the current paragraph"
  (re-search-forward "^\\s-*$" (point-max) "to_limit")
  (beginning-of-line))
...
;; $Id: snag.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
is consistent with the parsing syntax of the programs, which read from
the top of the input file and overwrite multiply encountered
keywords."
  (interactive "sWhich keyword?  ")
  (save-excursion
...
;; $Id: files.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
;;    input-jump-to-log-file ()
;;    input-log-file-exists-p (&optional ext)
...
;;    ease-toggle-hide-autoconfig (&optional switch)
;;    ease-hide-autoconfig ()
;;    ease-toggle-mouse-overlays (&optional switch)
;;    ease-set-all-mouse-overlays ()
;;    ease-set-mouse-overlays (list)
;;    ease-mark-hotspots (list)
;;    ease-clear-hotspot-overlays ()
;;    ease-clear-overlays (prop value)
...
angle brackets.  A uwxafs binary file will be displayed.  A file
...
(defun ease-toggle-hide-autoconfig (&optional switch)
...
if SWITCH < 0, toggle if switch is nil."
  ...
  (cond ((and (numberp switch) (> 0 switch))
	 (setq ease-hide-autoconfig-flag t))
	((and (numberp switch) (< 0 switch))
	 (setq ease-hide-autoconfig-flag nil))
	(t
	 (setq ease-hide-autoconfig-flag (not ease-hide-autoconfig-flag))))
  (if ease-hide-autoconfig-flag
      (ease-hide-autoconfig)
    (widen)))

(defun ease-hide-autoconfig ()
  (if ease-hide-autoconfig-flag
      (progn
	(let ((case-fold-search t)
...
(defun ease-toggle-mouse-overlays (&optional switch)
...
if SWITCH < 0, toggle if switch is nil."
  (interactive)
  (cond ((and (numberp switch) (> 0 switch))
	 (setq ease-mouse-overlays-flag t))
	((and (numberp switch) (< 0 switch))
	 (setq ease-mouse-overlays-flag nil))
	(t
	 (setq ease-mouse-overlays-flag (not ease-mouse-overlays-flag))))
  (if ease-mouse-overlays-flag
      (ease-set-all-mouse-overlays)
    (ease-clear-overlays 'mouse-face 'ease-mouse-face)))

(defun ease-set-all-mouse-overlays ()
  "Set the mouse overlays/extents in this file.
See `ease-mouse-highlight-list'."
  (interactive)
  (when ease-mouse-overlays-flag
    (ease-clear-overlays 'mouse-face 'ease-mouse-face)
    (ease-clear-overlays 'face 'ease-mouse-face)
    (map nil 'ease-set-mouse-overlays ease-mouse-highlight-list)
    (if (interactive-p) (message "Mouse flashing set")))
  nil)

(defun ease-set-mouse-overlays (list)
	  ...
	  (when (or (and (not (input-comment-p))
	    ...
	    (cond (ease-xemacs-p
		   (let ((extent (make-extent begin end)))
		     ...
		     (set-extent-mouse-face extent 'ease-mouse-face)
		  ...
		  (t
		   (overlay-put (make-overlay begin end)
				'mouse-face 'ease-mouse-face))))
	  (forward-line 1) )))))

;; why does this not obey the input-blank-p exception?
(defun ease-mark-hotspots (list)
...
good idea to do \\[ease-clear-hotspot-overlays] after filling in the template."
  (interactive)
  (save-excursion
    (while list
      (let ((marker (car list)))
	      ...
	      (unless (or (bobp) (eobp) (input-blank-p))
		      (cond (ease-xemacs-p
			     ...
			     (let* ((begin (1- marker))
			       ...
			       (set-extent-endpoints extent begin end)
			       (set-extent-face extent 'ease-hotspot-face)
			       (set-extent-property extent 'start-open t)
			       (set-extent-property extent 'end-open t)
			       (set-extent-property
				extent 'help-echo
			    ...
			    (t
			     (overlay-put (make-overlay (1- marker) marker)
					  'face 'ease-hotspot-face))))))
      (setq list (cdr list))	)))

;; want a key binding so...
(defun ease-clear-hotspot-overlays ()
  (interactive)
  (ease-clear-overlays 'face 'ease-hotspot-face)
  nil)

(defun ease-clear-overlays (prop value)
...
;; $Id: bug.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
	 ...
	 (reporter-submit-bug-report
	  ...
	  (append      ; variables to display values of in mail
	   '(input-load-hook
	     ...
	     input-never-convert-autoconfig-flag
	     ...
	     input-time-stamp-format
	     input-use-imenu
	     ease-doc-load-hook
	     ease-doc-mode-hook
	     ease-doc-idle-delay
	     ease-doc-identifier-string
	     ease-doc-mode
	     Atoms-evaluation-comment-string
	     Atoms-definition-comment-string
	     ease-gnuplot-beep-flag
	     ease-gnuplot-history-length
	     ease-gnuplot-top-bottom-separator
	     Atoms-load-hook
	     Atoms-mode-hook
	     Feff-load-hook
	     Feff-mode-hook
...
The mail will be sent to the maintainer of the program.  Use this
function only for submitting bug reports about feff or the analysis
software.  Use \\[input-submit-feedback] to submit a bug report about
EASE.  Bound to \\[input-submit-program-bug-report]"
  (interactive)
  (let ((line (make-string 70 ?-)) (file (buffer-file-name)))
    ...
    (and (y-or-n-p (format
	 ...
	 (reporter-submit-bug-report
	  ...
	  (format
	   "Bug report about %s
         report form generated by EASE (version %s)"
	  ...
	  (concat line                  ; salutation
		  ...
		  "     make the executable.\n\n"
		  "If you used the program execution features of EASE, you\n"
...
;; (defconst ease-ftp-site "atropos.nist.gov/pub/bruce/ease/"
;;   "Main FTP site with latest version of EASE.")
;; (defun ease-visit-home ()
;;  "Jump to the main FTP site for `ease-doc'."
;;  (interactive)
;;  (if (string-match "XEmacs" emacs-version)
;;     (require 'efs)
;;   (require 'ange-ftp))
;;  (require 'dired)
;;  (if input-use-frames
;;      (dired-other-frame ease-ftp-site)
;;    (dired-other-window ease-ftp-site)))
...
;; $Id: main.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $


;; functions defined in this section
;;    input-read-init-file (&optional rcfile)
;;    input-set-configuration ()
;;    input-write-configuration ()
;;    input-convert-old-configuration ()
;;    input-end-newline ()
;;    input-save-run-log (&optional arg)
;;    input-visit-previous-run-log ()
;;    input-ease-document ()
;;    input-ease-tutorial ()
;;    input-atoms-document ()
;;    input-feff-document ()
;;    input-normal-document ()
;;    input-phit-document ()
;;    input-document (&optional doc)
;;    input-parse-for-value (vble)
;;    input-determine-program (&optional prog)
;;    input-turn-off-minor-mode (mode)
;;    input-show-version ()
;;    input-set-time-stamp ()
;;    input-emulation ()
;;    ease-unsaved-message (&rest args)
;;    ease-display-startup-message (&optional arg)
...
(defvar input-autoconfig-alist '((input-program-name    . nil)
  ...
  "Alist of variables auto-configured by EASE.")
...
(defun input-read-init-file (&optional rcfile)
  "Read the *EASE* initialization file.
Typically this is `.ease' in the user's home directory
...
reset using this function.  Bound to \\[input-set-configuration]"
  ...
  (message "EASE is now reconfigured in this buffer.")
  nil)
...
`input-prohibit-autoconfig-flag' is t."
  ...
  (unless input-prohibit-autoconfig-flag
    (save-excursion
      (widen)
      (save-excursion
	(let ((case-fold-search t) begin end
...
assure that the programs correctly read the last line of the
input file."
  (save-excursion
...
(defun input-visit-previous-run-log ()
  "Examine the run-time log file from your previous *EASE* session.
Bound to \\[input-visit-previous-run-log]"
  ...
  (cond ((file-readable-p input-run-log)
	...
	(t
	 (message "%S does not exist.  Have you ever run EASE?"
		  input-run-log))) )

;;;###autoload
(defun input-ease-document ()
  "Look at on-line documentation for *EASE* using info or the w3 package."
  (interactive)  (input-document "ease"))
(defun input-ease-tutorial ()
  "Look at on-line documentation for *EASE* using info or the w3 package."
...
;; (if ease-xemacs-p
;;     (add-to-list 'paths-default-info-directories input-document-location)
;;   (add-to-list 'Info-default-directory-list input-document-location))
...
minibuffer."
  (interactive)
  (setq doc (or doc "ease-dir"))
  (let ((location input-document-location) file)
    (cond (doc
...
(defun input-determine-program (&optional prog buff)
	...
	(cond ((or (interactive-p) (not ease-always-start-generic-flag))
      ...
      (cond ((member input-program-name program-list)
	    ...
	    (t
	     ...
	     (setq input-program-name current-program)
	     (throw 'return-early nil)))
      (message "EASE: Loading %s mode" input-program-name)

					; load appropriate file and set stuff
      (setq mode  (cdr (assoc input-program-name input-programs-alist)))
      (setq feature (intern (concat "ease-" input-program-name)))
      (require feature)
      (funcall mode t)
      (setq input-current-feature feature)
      (message "EASE: Setting up highlighting")
      (funcall input-program-hilit)
      (cond (input-use-font-lock
...
(defun input-show-version ()
  "Show EASE version number in echo area."
  (interactive)
  (message "EASE version %s by %s, <%s>"
...
;; crisp, or edt, then EASE will be sure to enter one of those emulation
;; modes.  This is necessary for users that never use emacs except to run
;; ease (if such a thing exists!) because they might not have a .emacs
...
;; 	 ;;(and (fboundp 'viper-mode) (setq viper-mode t))
;; 	 ;;(and (fboundp 'vip-mode)   (setq vip-mode t))
...
(defun ease-unsaved-message (&rest args)
  (let ((message-log-max nil))
    (apply (function message) args)))
(defun ease-display-startup-message (&optional arg)
  (interactive)
  (if (or (interactive-p) (sit-for 1))
      (let ((lines ease-startup-message-lines))
	(message "EASE version %s.  Copyright (C) 1998-2000 %s <%s>"
		 input-mode-version input-author input-author-email)
	(if arg (setq lines (cons arg lines)))
	(while (and (sit-for 4) lines)
	  (message (substitute-command-keys (car lines)))
	  (setq lines (cdr lines)))))
  (ease-unsaved-message ""))
;; ---------------------------------------------------------------------
...
(defun input-set-local-variables ()
  ...
  (let ((vble-list '(input-comment-list
		     ...
		     input-time-stamp-line-limit ease-mouse-overlays-flag
		     ease-running-this-file ease-mouse-highlight-list)))
    (map nil 'make-local-variable vble-list)
    (defvar input-gnuplot-terminal input-gnuplot-default-terminal)    ))


(defun input-speedbar-p ()
  "True if the input file is chosen using speedbar."
  (interactive)
  (and (boundp 'speedbar-frame)
       (not (string-match "dead" (format "%S" speedbar-frame)))))


;;;###autoload
(defun input-mode ()
  "Major mode for editing input files for *feff*, *feffit*, and other
...
EASE (The EXAFS Analysis System for Emacs) is the work of Bruce Ravel
<ravel@phys.washington.edu> and is distributed free of charge to users
of Feff and FEFFIT (and anyone else who, for some reason, wants it).
This software comes with absolutely no warrenty (\\[describe-copying]).
Any suggestions, comments, criticisms, or contributions to EASE are
gratefully accepted.  Use \\[input-submit-feedback] to make a bug
report or in any other way communicate with EASE's author.

Current version numbers of programs at the time that this EASE
distribution was assembled are:
         program   version number        document on-line in EASE?
         -------   ---------------       ---------------------------
         atoms     2.46b and higher             yes
         feff      6.01, 7.02, 8xNN             yes (7.02)
         autobk    2.61 and higher              no
         feffit    2.32 and higher              no

Defined keys in input-mode:\n\\{input-mode-map}"

  (interactive)
  (message "EASE: Starting input mode")
  ...
  (unless input-init-file-loaded
    (message "EASE: Reading init file")
    (input-read-init-file)
    (setq input-init-file-loaded t))
  (message "EASE: Setting local variables")
  (input-set-local-variables)
  ...
  (message "EASE: Determining minor mode")
  (cond ((input-speedbar-p)
	 (let (buff)
	   (setq buff (current-buffer))
	   ;(select-frame speedbar-attached-frame)
	   (switch-to-buffer buff)
	   (let ((prog (input-parse-for-value "input-program-name")))
	     (input-determine-program prog buff))))
	(t
	 (switch-to-buffer (current-buffer))
	 (let ((prog (input-parse-for-value "input-program-name")))
	   (input-determine-program prog (current-buffer)))))
  (redraw-frame (selected-frame))
  (if (string-match "quit" input-program-name)
      ;; bail out of input-mode
      (text-mode)
    (message "") ; crufty hack to get menu- and toolbars redisplayed properly
    ...
    (add-hook 'write-contents-hooks 'ease-clear-hotspot-overlays nil t)
    ...
    (setq time-stamp-active     t
	  ...
	  time-stamp-format     input-time-stamp-format)
    ...
    (setq font-lock-defaults '((input-font-lock-keywords
				...
				input-font-lock-keywords-2)
			       t t ((?\_ . "w"))))
    (message "EASE: Setting mouse overlays")
    (if ease-mouse-overlays-flag (ease-set-all-mouse-overlays))
    (when imenu-generic-expression
      (message "EASE: Setting up imenu")
      (and input-use-imenu
	   (condition-case ()
	       (require 'imenu)
	     (error nil)))
      (and (featurep 'imenu)
	   (imenu-add-to-menubar input-use-imenu)))
;    (when ease-xemacs-p			; perform a complete redisplay
;      (redraw-frame (window-frame) t))	; so that the tool- and menubars
;;      (sit-for 0))			; are properly displayed
    (run-hooks 'input-mode-hook)
    (when (and (not ease-inhibit-startup-message)
	       (not ease-startup-displayed))
	       ;;(or (memq this-command 'input-mode))
		;;   (null last-command))
	  (if ease-xemacs-p
	      (add-timeout 1 'ease-display-startup-message
			   (concat "Use the toolbar to make templates,"
				   " run, plot and jump to log files."))
	    (run-at-time 1 nil 'ease-display-startup-message))
	  (setq ease-startup-displayed t))
