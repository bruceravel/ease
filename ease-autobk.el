;;; ease-autobk.el --- minor mode for editing autobk input files
...
;; $Id: ease-autobk.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
(defcustom Autobk-load-hook nil
  "*Hook run when ease-autobk.el is first loaded."
  :group 'ease-programs
  :type 'hook)
...
(defun Autobk-parse-stanza ()
  ...
  (save-excursion
	...
	(setq Autobk-features-alist nil
	      eos (save-excursion (input-end-of-stanza) (point-marker)))
...
not given, stanza will parsed for data file name."
  ...
  (let (datafile (list () ) ymin ymax pad scratch)
    ...
    (cond ((file-exists-p datafile)
	   (setq scratch (make-temp-name "EASE-scratch"))
	 ...
	 (if Autobk-mode
	     (progn
	       ...
	       (setq imenu-generic-expression Autobk-imenu-generic-expression
		     ...
		     ease-mouse-highlight-list Autobk-highlight-list)
