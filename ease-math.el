;;; ease-math.el --- using calc with Atoms mode

;; Author:  Bruce Ravel <ravel@phys.washington.edu>
;; Maintainer:  Bruce Ravel <ravel@phys.washington.edu>
;; CReated: 10 May 1998
;; Version:  0.6.5
;; Keywords:  feff, input files, atoms, calc

;; $Id: ease-math.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
;; the file.  This can be done by putting the following in the .ease
;; file:
    ...
    (cond ((input-evaluation-p)
	   ...
	   (let (elem (xyz ()) (tag " ") (rest " ") begin sexp space check
	     ...
	     (when (and (string= tag check)
	       ...
	       (delete-char 1))
	     (insert (format "%s%-2s\t%7.4f\t%7.4f\t%7.4f\t%-10s\t%7.4f  %s\n"
			     space elem (elt xyz 0) (elt xyz 1)
			     (elt xyz 2) tag (elt xyz 3) rest)) ))
	  (t
...
;; conditional on (featurep 'calc) in ease-toolbar.
