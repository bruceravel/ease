;;; ease-feff.el --- minor mode for editing feff input files
...
;; $Id: ease-feff.el,v 1.1.1.1 2000/05/21 05:46:16 bruce Exp $
...
(defcustom Feff-load-hook nil
  "*Hook run when ease-feff.el is first loaded."
  :group 'ease-programs
  :type 'hook)
...
(setq Feff-keywords-alist '(
      ...
      ("fms" .
       (3 "number"
	  "three real numbers (scf/full cluster size, kmax for fms)"))
      ("folp" .
       (2 ("integer" "number")
      ...
      ("s02" .
       (1 "number"
	  "a real number (the amplitude reduction factor)"))
      ("scf" .
      ...
      ("xanes" .
       (1 "number"
	  "max for the xanes calculation"))
      ("exafs" .
       (1 "number"
	  "max for the exafs calculation"))
...
parameters will be inserted in the template.  Bound to \\[Feff-make-template]."
  ...
  (ease-mark-hotspots tempo-marks)
  ...
  (let (title sigmm qrtmm ampfac mark-1 (case-fold-search t))
    (save-excursion
      ...
      (cond ((re-search-backward
	      "^[ \t]*\\*[ \t]*\\(sum\\|mcmaster\\|Normalization\\)"
	      (point-min) t)
	     (re-search-forward "corrections?:")
	     (re-search-forward input-word-sep)
	     (setq mark-1 (point))
	     (re-search-forward input-word-sep)
...
Feff does not really have a log file so show the results of intrp."
  (interactive)
...
;; Feff-8-write-convergence-data can be used this way in the .ease file:
...
;; 	      (card-list (list "afolp" "scf" "fms" "rpath" "ldos")))
...
;;				 "rmax" "rpath" "debye" "rmultiplier" "ss"
;;				 "nleg" "criteria" "nogeom" "csig"
;;				 "iorder" "pcriteria" "sig2" "xanes" "exafs"
...
;;     ("^[ \t]*control\\([ \t]+[0-9]\\)+[ \t]*" . Feff-control-face)
    ...
    ("^[ \t]*\\(control\\|end\\|print\\)\\>" . font-lock-type-face)
					; keywords
    ("^[ \t]*\\(afolp\\|c\\(orrections\\|riteria\\|sig\\|ustomcrit\\)\\|d\\(crit\\|ebye\\)\\|e\\(dge\\|grid\\|llipticity\\|mesh\\|x\\(afs\\|change\\)\\)\\|f\\(ms\\|o\\(lp\\|rce\\)\\)\\|hole\\|i\\(nterstitial\\|o\\(n\\|rder\\)\\)\\|jumprm\\|ldos\\|m\\(bconv\\|inx\\)\\|n\\(emax\\|legs?\\|o\\(geom\\|hole\\)\\|star\\)\\|p\\(criteria\\|olarization\\)\\|r\\(grid\\|m\\(ax\\|ultiplier\\)\\|p\\(ath\\|hases\\)\\)\\|s\\(02\\|cf\\|ig[23]\\|pin\\|s\\)\\|vintfix\\|x\\(anes\\|ion\\)\\)\\>" . font-lock-keyword-face) ))
	 ...
	 (hilit-set-mode-patterns
	  ...
	  '(
	    ...
	    ("^[ \t]*\\(print\\|control\\|end\\)\\>" nil glob-struct)
					; all the rest
	    ("^[ \t]*\\(afolp\\|c\\(orrections\\|riteria\\|sig\\|ustomcrit\\)\\|d\\(crit\\|ebye\\)\\|e\\(dge\\|grid\\|llipticity\\|mesh\\|x\\(afs\\|change\\)\\)\\|f\\(ms\\|o\\(lp\\|rce\\)\\)\\|hole\\|i\\(nterstitial\\|o\\(n\\|rder\\)\\)\\|jumprm\\|ldos\\|m\\(bconv\\|inx\\)\\|n\\(emax\\|legs?\\|o\\(geom\\|hole\\)\\|star\\)\\|p\\(criteria\\|olarization\\)\\|r\\(grid\\|m\\(ax\\|ultiplier\\)\\|p\\(ath\\|hases\\)\\)\\|s\\(cf\\|ig[23]\\|pin\\|s\\)\\|vintfix\\|x\\(anes\\|ion\\)\\)\\>" nil keyword)
	 ...
	 (if Feff-mode
	     (progn
	       ...
	       (setq imenu-generic-expression Feff-imenu-generic-expression
		     ...
		     ease-mouse-highlight-list Feff-highlight-list)
